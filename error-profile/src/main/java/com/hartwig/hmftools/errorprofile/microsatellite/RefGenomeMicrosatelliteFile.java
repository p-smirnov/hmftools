...
import com.hartwig.hmftools.common.utils.file.TabularFileWriter;
...
public class RefGenomeMicrosatelliteFile implements AutoCloseable
{
    ...
    private final TabularFileWriter<RefGenomeMicrosatellite> mWriter;

    public RefGenomeMicrosatelliteFile(String fileName)
    {
        mWriter = new TabularFileWriter<>(fileName, Arrays.stream(Column.values()).map(Enum::name).collect(Collectors.toList()),
    ...
    {
        List<RefGenomeMicrosatellite> refGenomeMicrosatellites = new ArrayList<>(18_000_000);
